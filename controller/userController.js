...
const createUser = async (req, res) => {
  try {
    ...
    keys.map((keys) => {
      if (!['email', 'password', 'name'].includes(keys))
        return res.status(400).send({
          status: false,
          message: `${keys} is not valid plese enter only email, password, name, dob`,
        });
    });
    ...
    if (!email)
      return res
        .status(400)
        .send({ status: false, message: 'email is not presnt' });
    if (!validAll(email))
      return res
        .status(400)
    ...
    if (!password)
      return res
        .status(400)
        .send({ status: false, message: 'password is not presnt' });
    if (!validAll(password))
      return res
        .status(400)
    ...
    if (!name)
      return res
        .status(400)
        .send({ status: false, message: 'name is not presnt' });
    ...
    if (!savedData)
      return res.status(500).send({ status: true, message: 'not created' });

    return res
      .status(200)
      .send({ status: true, message: 'user is created', data: savedData });
  } catch (error) {
...
const loginUser = async function (req, res) {
  try {
    ...
    if (!getUser)
      return res
        .status(404)
        .send({ status: false, msg: 'Email or Password is Invalid' });
